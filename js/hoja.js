$(document).ready(() => {
    ...
    const featuresSelected = JSON.parse(localStorage.getItem('featuresSelected'));
    mostrarCaracteristicas(featuresSelected);
});
...
$('#btnEnviarCorreo').click(() => {
    ...
    html2pdf().set(opciones).from(element).toPdf().output('datauristring')
        .then(pdfBase64 => {
            $.post('https://masgps-bi.wit.la/certificado-wit/server/send_mail.php', { pdfBase64, email })
